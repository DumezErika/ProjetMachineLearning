...
class ConcentricSquares(Dataset):
    """Dataset of concentric d-dimensional squares. Points in the inner square
    ...
    num_points_outer : int
        Number of points in outer cluster
    """
    def __init__(self, inner_range, outer_range, num_points_inner,
                 num_points_outer):
        self.inner_range = inner_range
        self.outer_range = outer_range
        self.num_points_inner = num_points_inner
        ...
        # Generate data for inner square
        for _ in range(self.num_points_inner):
            self.data.append(
                random_point_in_square(inner_range[0], inner_range[1])
            )
            self.targets.append(torch.Tensor([-1]))
        ...
        for _ in range(self.num_points_outer):
            self.data.append(
                random_point_in_square(outer_range[0], outer_range[1])
            )
            self.targets.append(torch.Tensor([1]))
...
def random_point_in_square(min_radius, max_radius):
    """Returns a point sampled uniformly at random from a square if min_radius
    is 0. Else samples a point approximately uniformly on annulus.
    Parameters
    ----------
    min_radius : float
    ...
    # Sample distance of point from origin
    unif = random()
    distance = (max_radius - min_radius) * (unif ** (1. / 2)) + min_radius
    # Sample direction of point away from origin
    direction = torch.randn(2)
    unit_direction = direction / torch.max(torch.abs(direction))
    return distance * unit_direction
